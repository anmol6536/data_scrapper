...
class request_handler():
    ...
    def get_urls(self,search_string, total_results = 25 ):
        #url generation
        base = f'https://api.elsevier.com/content/search/scopus?start=0&count=25&query={search_string}&apiKey={self.apikey}'
        start=0
        end=round(total_results/25)
        urls = []
        for i in range(end+1):
            url = f'https://api.elsevier.com/content/search/scopus?start={start}&count=25&query={search_string}&apiKey={self.apikey}'
            start+=25
            urls.append(url)
        return urls
